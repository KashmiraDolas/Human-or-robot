...
def load():
	...
	b = bot_or_human[['bidder_id', 'payment_account']].groupby(
		'payment_account').size().reset_index().sort_values(0, ascending=False)
...
def dt(X, bids):
	...
	'''
	for week in [0, 2, 4]:
		b = bids[bids.week==week].sort_values(['auction', 'time'])[['auction',
															  'time', 'bidder_id']]
	...
	for week in [0, 2, 4]:
		b = bids[bids.week==week].sort_values(['bidder_id', 'time'])[[
			'bidder_id', 'time']]
...
def bid_order(X, bids):
	...
	a = bids.sort_values('time', ascending=True).groupby('auction',
...
# 	a.groupby(['bidder_id']).sort_values('c_count').agg(lambda x: x.iloc[0])
# 	
...
def make(X):
	...
	#X = X.sort_values(['n_bids', 'dt_others_median'])
	X = X.sort_values(['dt_others_median'])
	#X = X.sort_values(['on_ip_that_has_a_bot_mean', 'dt_others_median'])
	X = X.fillna(method='pad')
#  	X = X.sort_values(['on_ip_that_has_a_bot_mean'])
#  	X = X.fillna(method='pad')
	...
	if len(scores) == 100:
		print (np.sort_values(np.array(scores))[10])
		print (np.sort_values(np.array(scores))[25])
		print (np.sort_values(np.array(scores))[50])
